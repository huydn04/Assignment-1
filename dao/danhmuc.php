...
function get_soluong_dm(){
    $sql = "SELECT COUNT(id) FROM danhmuc WHERE 1";
...
function get_danhmuc_form($dsdm){
    $html_dm = '';
    foreach($dsdm as $dm){
        $html_dm .= '
...
function get_sethome_dm($id) {
    $sql = "SELECT sethome FROM danhmuc WHERE id=?";
    return pdo_query_value($sql, $id);
}
?>
