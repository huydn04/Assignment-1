...
function get_dssp_new($limit){
    $sql = "SELECT  sp.id, dm.ten_loai, sp.hinh, sp.gia, sp.giam_gia, sp.so_luot_xem, sp.ten_sp 
            FROM sanpham sp 
            INNER JOIN danhmuc dm ON sp.id_catalog = dm.id 
            WHERE sp.ngay_nhap 
...
function get_dssp_best($limit){
    $sql = "SELECT  *, sp.id 
            ...
            ORDER BY sp.so_luot_xem DESC LIMIT ".$limit;
    return pdo_query($sql);
}
    ...
    if (is_array($dssp) && count($dssp) > 0) {
        foreach ($dssp as $sp) {
            ...
            $html_dssp .= '<div class="pro' . (isset($sp['dac_biet']) ? ' special' : '') . '">
                              ...
                              <div class="des">
                                  <span class="category">' . (isset($sp['ten_loai']) ? $sp['ten_loai'] : '') . '</span>
